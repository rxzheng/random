...
class UserProfile:
    def __init__(self, dict):
        ...
        try:
            self.__protdict__.update(
                userProfile["entry_data"]["ProfilePage"][0]["graphql"]["user"]
            )
        except Exception:
    ...
    def __repr__(self):
        try:
            ...
            username = ""
        return (
            "userProfile( username='{0}' )".format(username)
            if username
            else "userProfile()"
        )
    ...
    def story(self, args=["Source", "iterlink"]):
        if args not in ["Source", "iterlink"]:
            return
        if args == "Source":
            return self.__inst__.getStoryDataByUserName(self.username)["reels_media"][
                0
            ]["items"]
...
class Instagram:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if os.path.lexists(os.path.join(os.path.dirname(__file__), "CacheData.cc")):
            try:
                with open(
                    os.path.join(os.path.dirname(__file__), "CacheData.cc"), "rb"
                ) as file:
    ...
    def login(self, username, password):
        ...
        if jsonify.get("authenticated"):
            ...
            with open(
                os.path.join(os.path.dirname(__file__), "CacheData.cc"), "wb"
            ) as file:
                pickle.dump(
                    {
    ...
    def story(self):
        ...
        return (
            self.fetch("getStoryByUserName")
            if self.cache.story
            else self.fetch("storytray")
        )
    ...
    def getJsonByUsername(self, username):
        ...
        user = json.loads(
            userdata.text.split("window._sharedData =")[1].split(";</script>")[0]
        )
        if user:
            return user
        return
                ...
                for ind, _ in enumerate(user["edge_owner_to_timeline_media"]["edges"]):
                    ...
                    if node["__typename"] == "GraphSidecar":
                        ...
                        for i, post in enumerate(posts):
                            if (post[i]["node"]["__typename"] == "GraphImage") and not (
                                post[i]["node"]["is_video"]
                            ):
